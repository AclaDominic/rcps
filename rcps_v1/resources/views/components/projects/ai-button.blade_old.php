...
<div x-data="{
   ...
   updateFields() {
        ...
        for (const id in components) {
            ...
            this.ai_subtask_count = taskData.ai_subtask_count || 0;
    ...
    async generateAI(mode) {
        ...
        if (this.ai_subtask_count === 0) {
            alert('Please enter number of subtasks');
            return;
        }
    ...
    <div class="flex gap-2">
        <!-- Generate AI Button -->
        <button type="button"
                x-on:click="generateAI('divide_conquer')"
                :disabled="isLoading || !project_name || !main_task_name || !main_task_description || ai_subtask_count === 0"
                :class="{
                    'bg-gray-400 cursor-not-allowed opacity-60': isLoading || !project_name || !main_task_name || !main_task_description || ai_subtask_count === 0,
                    'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all': !isLoading && project_name && main_task_name && main_task_description && ai_subtask_count > 0
                }"
            ...
            <span class="whitespace-nowrap">
                <span x-show="!isLoading" 
                      x-text="!project_name || !main_task_name || !main_task_description || ai_subtask_count === 0 ? 
                              'Fill All Fields (D&C)' : 
                              'Generate AI Subtasks (D&C)'"></span>
                <span x-show="isLoading">Generating...</span>
            </span>
            
            <!-- Badge -->
            <span x-show="!isLoading && project_name && main_task_name && main_task_description && ai_subtask_count > 0" 
         ...
         <button type="button"
            ...
            <span class="whitespace-nowrap">
                <span x-show="!isLoading" 
                      x-text="!project_name || !main_task_name || !main_task_description || ai_subtask_count === 0 ? 
                              'Fill All Fields (Greedy)' : 
                              'Generate AI Subtasks (Greedy)'"></span>
                <span x-show="isLoading">Generating...</span>
            </span>
    ...
    <x-filament::modal  id="{{$comparativeModalId}}" width="7xl" :scrollable="true">
        ...
        <div class="space-y-6" x-data="{ 
            ...
            countQuickWins(algorithm) {
                const tasks = this.comparative_preview?.algorithms?.[algorithm]?.tasks || [];
                return tasks.filter(t => t.estimated_hours <= 2).length;
            },
            isDivideConquerRecommended() {
                const dividerHours = this.getDividerHours();
                const greedyHours = this.getGreedyHours();
                return dividerHours < greedyHours;
            },
            ...
            getParallelizationComparison() {
                const dcTasks = this.comparative_preview?.algorithms?.divide_conquer?.tasks || [];
                const greedyTasks = this.comparative_preview?.algorithms?.greedy?.tasks || [];
                const dcParallel = dcTasks.filter(t => t.parallelizable).length;
                const greedyParallel = greedyTasks.filter(t => t.parallelizable).length;
                return greedyParallel > dcParallel ? 'Greedy better' : 'D&C better';
            },
            getRiskComparison() {
                const dcTasks = this.comparative_preview?.algorithms?.divide_conquer?.tasks || [];
                const greedyTasks = this.comparative_preview?.algorithms?.greedy?.tasks || [];
                        ...
                        <div class="p-4 bg-green-50 rounded-lg border border-green-200">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold text-green-700" 
                                    x-text="(comparative_preview?.algorithms?.greedy?.tasks || []).filter(t => t.estimated_hours <= 2).length"></p>
                                </div>
                                <div class="p-2 bg-green-100 rounded-full">
                                    <svg class="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>
                                                ...
                                                <div class="flex items-center">
                                                    ...
                                                    <div class="ml-3">
                                                        ...
                                                        <div class="text-xs text-gray-500">
                                                            ID: <span x-text="task.responsible_id || 'N/A'"></span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </template>
                    ...
                    <div class="p-6">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-gray-700 bg-gray-50 p-3 rounded" x-text="selectedTask?.description"></p>
                            </div>
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-3">
                                                ...
                                                <p class="text-sm text-gray-500">
                                                    User ID: <span x-text="selectedTask?.responsible_id"></span>
                                                </p>
                                            </div>
                                        </div>
                                    </template>
