<div x-data="{
    ...
    updateFields() {
        ...
        for (const id in components) {
            ...
            if (!formData || !formData.add_task) continue;
            
            // Get data for THIS specific repeater
            const taskObject = formData.add_task;
            this.taskData = Object.values(taskObject);
    ...
    // Add this function to check if taskData has valid content
    isTaskDataValid() {
        // Check if taskData exists and has items
        if (!this.taskData || this.taskData.length === 0) {
            return false;
        }
        
        // Check if at least one item has subtask_title with content
        const hasValidItem = this.taskData.some(item => {
            return item && 
                   item.subtask_title && 
    ...
    // Check if all required fields are filled AND taskData is valid
    canGenerateAI() {
    ...
    // Check if there are any empty subtask_title fields
    hasEmptySubtaskNames() {
        if (!this.taskData || this.taskData.length === 0) {
            return false;
        }
        
        return this.taskData.some(item => {
            return !item || 
                   !item.subtask_title || 
                   item.subtask_title.trim() === '';
        });
    ...
    getEmptySubtasksInfo() {
        ...
        this.taskData.forEach((item, index) => {
            if (!item || 
                ...
                emptyIndices.push(index + 1); // +1 para human readable (1-based)
            }
        });
    ...
    getSubtasksAlertMessage() {
        ...
        if (emptyInfo.hasEmpty) {
            ...
            return `Please fill in ${subtaskWord} #${indicesList} or remove ${emptyInfo.emptyIndices.length === 1 ? 'it' : 'them'} to proceed.`;
        }
    ...
    async generateAI() {
        if (!this.project_id || !this.name || !this.content) {
            alert('Please fill all required fields (Project, Task Name, and Description)');
            return;
        }
            ...
            <!-- Text -->
             <span class="whitespace-nowrap">
                <span x-show="!isLoading"  
                    x-text="!project_id && !name && !content && !isTaskDataValid() ? 'Fill All Fields':'Generate Computation'">
                </span>
        ...
        <x-slot name="heading">
            <div class="flex items-center justify-between w-full">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-xl font-bold text-gray-900 dark:text-white">Task Computation Preview</h2>
        ...
        <div class="max-h-[70vh] overflow-y-auto">
                ...
                <div class="space-y-6 p-1">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border dark:border-gray-700 overflow-hidden">
                        <div class="px-6 py-4 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-300">Detailed breakdown</p>
                        </div>
            ...
            <template x-if="!preview">
                <div class="text-center py-16">
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">No Tasks Generated Yet</h3>
                    <p class="text-gray-600 dark:text-gray-300 mb-6">Generate subtasks first to see the computation preview.</p>
